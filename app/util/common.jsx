...
module.exports = {
  ...
  addZero: function addZero(num) {
    const inte = Math.abs(num);
    if (num < 0) return (inte < 10) ? ('-0' +inte) : num;
    return (inte < 10) ? ('0' +inte) : num;
  },
  getUtcOffset: function getUtcOffset(offset) {
    return ' (GMT' + ((offset > 0) ? '+' : '') + this.addZero(parseInt((offset/1000)/3600)) + ':' + this.addZero(((offset/1000)%3600)/60) + ')';
  },
  ...
  localTimestampToGMT : (localTimestamp) =>{
    let date  = new Date();
    let GMTTimestamp = new Date(localTimestamp + date.getTimezoneOffset() * 60000).getTime();
    console.log(localTimestamp,GMTTimestamp);
    return GMTTimestamp;
  },
  GMTTimeStampToLocal : (GMTTimestamp) =>{
    let date  = new Date();
    let localTimestamp = new Date(GMTTimestamp - date.getTimezoneOffset() * 60000).getTime();
    console.log(localTimestamp,GMTTimestamp);
